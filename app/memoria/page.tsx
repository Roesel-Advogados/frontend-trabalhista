    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 font-display text-3xl font-bold text-wine-700">{lista.length}</p>
        </div>
        <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
          <p className={LC}>Contestações</p>
          <p className="mt-1 font-display text-3xl font-bold text-stone-700">{contestacoes}</p>
        </div>
        <div className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
          <p className={LC}>Recursos</p>
          <p className="mt-1 font-display text-3xl font-bold text-stone-700">{recursos}</p>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex-1 min-w-[200px]">
            ...
            <button onClick={() => fileRef.current?.click()}
              className="mt-1 flex w-full items-center gap-2 rounded-xl border border-dashed border-wine-200 bg-wine-50/50 px-4 py-2.5 text-sm font-medium text-wine-700 hover:border-wine-400 transition">
          ...
          <div>
            ...
            <select value={tipo} onChange={(e) => setTipo(e.target.value)}
              className="mt-1 block rounded-xl border border-stone-200 bg-stone-50 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-wine-500">
              <option value="contestacao">Contestação</option>
              <option value="recurso">Recurso</option>
            </select>
          </div>
          <button onClick={enviar} disabled={!file || loading}
            className="flex items-center gap-2 rounded-xl bg-wine-600 px-5 py-2.5 text-sm font-bold uppercase tracking-wide text-white hover:bg-wine-700 transition disabled:opacity-40">
      ...
      <div className="overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm">
        ...
        {carregandoLista ? (
          <div className="flex justify-center py-12"><Loader2 size={28} className="animate-spin text-wine-600" /></div>
        ) : lista.length === 0 ? (
          ...
          <ul className="divide-y divide-stone-100">
            {lista.map((d) => (
              <li key={d.id} className="flex items-center gap-3 px-6 py-3">
                <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-wine-50 text-wine-700">
